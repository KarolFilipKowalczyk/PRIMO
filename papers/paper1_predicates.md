...
*Remark (Signature as complexity proxy).* The PRIMO program enumerates rules by signature level σ. A DPO rule at signature σ → (σ+1) has Kolmogorov complexity K(ρ) = Θ(σ²): the upper bound follows from a canonical encoding of the triple (L, R, ι) requiring σ² + O(σ log σ) bits; the lower bound follows from counting — the number of distinct rules at level σ is 2^{Θ(σ²)}, so most rules require Ω(σ²) bits. Within-level variation in K(ρ) is O(σ log σ), sub-leading compared to the σ² inter-level gap. This justifies stating the PRIMO conjecture at the level of σ rather than individual K(ρ). Combined with the compression gate (Proposition 3, Section 7.5), the information content of an entire trajectory is K(trajectory) ≤ O(σ²) + K(G₀) + O(log T), bridging the predicate framework to complexity-theoretic foundations. See [3, Section 6.3] for the full lemma.
